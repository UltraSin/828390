...
-- Auto-detect: scan ReplicatedStorage for any RemoteEvent that looks like a hit remote
local function FindHitRemotes()
	local found = {}
	for _, obj in ipairs(game.ReplicatedStorage:GetDescendants()) do
		if obj:IsA("RemoteEvent") then
			local n = obj.Name:lower()
			if n:find("hit") or n:find("punch") or n:find("damage") or n:find("swing") or n:find("strike") then
				found[obj.Name] = obj
			end
		end
	end
...
local _cachedHitRemotes = nil
local _cachedGloveRemote = nil
local _cachedGloveName = nil

local function GetGloveRemote()
	local player = game.Players.LocalPlayer
	local leaderstats = player:FindFirstChild("leaderstats")
	local gloveObj = leaderstats and leaderstats:FindFirstChild("Glove")
	local gloveName = gloveObj and gloveObj.Value or ""

	-- Return cached remote if the glove hasn't changed
	if gloveName == _cachedGloveName and _cachedGloveRemote then
		return _cachedGloveRemote
	end

	-- Rebuild remote cache if needed
	if not _cachedHitRemotes then
		_cachedHitRemotes = FindHitRemotes()
	end

	-- Try to find a remote whose name contains the glove name (case-insensitive)
	if gloveName ~= "" then
		local gloveLower = gloveName:lower()
		for remoteName, remote in pairs(_cachedHitRemotes) do
			if remoteName:lower():find(gloveLower) or gloveLower:find(remoteName:lower():gsub("hit",""):gsub("punch",""):gsub("damage",""):gsub("swing",""):gsub("strike","")) then
				_cachedGloveName = gloveName
				_cachedGloveRemote = remote
				return remote
			end
		end
	end

	-- Fallback: pick any detected hit remote, preferring ones named "GeneralHit" or "Hit"
	local fallback = _cachedHitRemotes["GeneralHit"]
		or _cachedHitRemotes["Hit"]
		or _cachedHitRemotes["KSHit"]
		or next(_cachedHitRemotes) and select(2, next(_cachedHitRemotes))
		or game.ReplicatedStorage:FindFirstChild("GeneralHit")
		or game.ReplicatedStorage:FindFirstChild("Hit")

	_cachedGloveName = gloveName
	_cachedGloveRemote = fallback
	return fallback
end

-- Invalidate cache when glove changes so next hit auto-detects fresh
local _gloveWatchPlayer = game.Players.LocalPlayer
local _gloveWatchLS = _gloveWatchPlayer:FindFirstChild("leaderstats")
if _gloveWatchLS then
	local _gloveVal = _gloveWatchLS:FindFirstChild("Glove")
	if _gloveVal then
		_gloveVal:GetPropertyChangedSignal("Value"):Connect(function()
			_cachedGloveName = nil
			_cachedGloveRemote = nil
			_cachedHitRemotes = nil
		end)
	end
end
