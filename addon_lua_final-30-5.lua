...
-- opts = { color, label }  (both optional)
local function CreateHitbox(size, offset, duration, opts)
	opts = opts or {}
	local player = game.Players.LocalPlayer
	local character = player.Character or player.CharacterAdded:Wait()
	...
	-- Build visible hitbox part
	local part = Instance.new("Part")
	part.Name = "HitboxVisual"
	part.Size = size
	part.Color = opts.color or Color3.fromRGB(255, 0, 0)
	...
	-- Outline stroke for the keyframe box look
	local selBox = Instance.new("SelectionBox")
	selBox.Adornee = part
	selBox.Color3 = opts.color or Color3.fromRGB(255, 0, 0)
	selBox.LineThickness = 0.06
	selBox.SurfaceTransparency = 1
	selBox.SurfaceColor3 = opts.color or Color3.fromRGB(255, 0, 0)
	selBox.Parent = part

	-- Label showing ability name above the box
	if opts.label then
		...
		lbl.Text = opts.label
		lbl.TextColor3 = opts.color or Color3.fromRGB(255, 0, 0)
...
CreateHitbox(Vector3.new(26, 38, 38), Vector3.new(0, -2, -4), 0.4, {color=Color3.fromRGB(255,140,0), label="Overhead"})
...
wait(0.2)
coroutine.wrap(SpawnAfterImage)()
CreateHitbox(Vector3.new(38, 48, 46), Vector3.new(0, 10, -5), 0.15, {color=Color3.fromRGB(0,220,255), label="Uppercut"})
task.delay(0.01, function() coroutine.wrap(SpawnAfterImage)() end)
end
local function BlueFlash()
local Players = game:GetService("Players")
...
coroutine.wrap(SpawnAfterImage)()
-- Die directly fires on the locked target since we're teleported right next to them
local dieRemote = GetGloveRemote()
local dieTorso = tChar:FindFirstChild("Torso") or tChar:FindFirstChild("UpperTorso")
if dieTorso then
	pcall(function() dieRemote:FireServer(dieTorso) end)
end
-- Small visual hitbox so the player can see each punch zone
CreateHitbox(Vector3.new(18, 18, 18), Vector3.new(0, 0, -2), 0.18, {color=Color3.fromRGB(255,40,40), label="Die"})
task.delay(0.01, function() coroutine.wrap(SpawnAfterImage)() end)
...
coroutine.wrap(SpawnAfterImage)()
CreateHitbox(Vector3.new(50, 20, 50), Vector3.new(0, -8, -2), 0.4, {color=Color3.fromRGB(180,0,255), label="Judgement"})
task.delay(0.01, function() coroutine.wrap(SpawnAfterImage)() end)
wait(0.7)
...
coroutine.wrap(BlueWave)()
coroutine.wrap(SpawnAfterImage)()
CreateHitbox(Vector3.new(56, 24, 56), Vector3.new(0, -6, 0), 0.4, {color=Color3.fromRGB(0,255,120), label="Crush"})
task.delay(0.01, function() coroutine.wrap(SpawnAfterImage)() end)
wait(0.5)
local P = game:GetService("Players").LocalPlayer
local C = P.Character or P.CharacterAdded:Wait()
...
local function PunchAbility()
	...
	snd:Play()
	snd.Ended:Connect(function() snd:Destroy() end)
	CreateHitbox(Vector3.new(14, 10, 14), Vector3.new(0, 0, -4), 0.2, {color=Color3.fromRGB(150,200,255), label="Punch"})
